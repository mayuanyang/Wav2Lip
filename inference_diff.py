...
parser = argparse.ArgumentParser(description='Inference code to lip-sync videos in the wild using Wav2Lip models with gradual denoising')
...
parser.add_argument('--diffusion_steps', type=int, help='Number of diffusion steps for gradual denoising', default=20)
...
def gradual_denoising(model, audio_sequences, face_sequences, diffusion_steps=20):
    """
    实现逐步去噪过程
    """
    # 初始化：使用输入图像作为初始状态
  ...
  for x in range(args.iteration):
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen, 
      ...
      with torch.no_grad():
        ...
        # 使用逐步去噪
        if args.version == 'V5':
            pred = gradual_denoising(model, mel_batch, img_batch, args.diffusion_steps)
        else:
            # 对于非V5版本，使用原来的推理方式
