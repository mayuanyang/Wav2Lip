...
class TransformerSyncnetV2(nn.Module):
    def __init__(self):
        ...
        self.face_proj = nn.Linear(768, 512)
        ...
        self.cross_attn = CrossAttention(embed_dim=512, num_heads=8, dropout=dropout)
    ...
    def forward(self, face_embedding, audio_embedding):
        ...
        face_to_audio = self.cross_attn(face_proj, audio_embedding, audio_embedding)  # (batch_size, embed_dim)
        audio_to_face = self.cross_attn(audio_embedding, face_proj, face_proj)  # (batch_size, embed_dim)
