...
class ResUNet384V8(nn.Module):
    def __init__(self, print_gradients=False):
        ...
        self.bottom_unet_bottleneck = nn.Conv2d(192, 256, kernel_size=3, stride=1, padding=1) #construct_encoder_layers(3, 192, 256, 1)
                
        
        # --- MaxPooling for residual connection from encoder1 to encoder3 ---
        self.encoder1_to_encoder2_pool = nn.MaxPool2d(kernel_size=2, stride=2)  # 2x downsampling
        self.encoder1_to_encoder2_conv = nn.Conv2d(24, 48, kernel_size=1)  # Channel adjustment
        

        # --- MaxPooling for residual connection from encoder1 to encoder4 ---
        self.encoder2_to_encoder3_pool = nn.MaxPool2d(kernel_size=2, stride=2)  # 2x downsampling
        self.encoder2_to_encoder3_conv = nn.Conv2d(96, 96, kernel_size=1)  # Channel adjustment
        
        self.encoder3_to_encoder4_pool = nn.MaxPool2d(kernel_size=2, stride=2)  # 2x downsampling
        self.encoder3_to_encoder4_conv = nn.Conv2d(192, 192, kernel_size=1)  # Channel adjustment
        
        
        self.encoder4_to_encoder5_pool = nn.MaxPool2d(kernel_size=2, stride=2)  # 2x downsampling
        self.encoder4_to_encoder5_conv = nn.Conv2d(192, 192, kernel_size=1)  # Channel adjustment
        
        self.bottom_down4_pool = nn.MaxPool2d(kernel_size=2, stride=2)  # 2x downsampling
        self.bottom_down4_conv = nn.Conv2d(192, 192, kernel_size=1)  # Channel adjustment
        ...
        self.audio1_conv = nn.Conv2d(32, 64, kernel_size=1)
        ...
        self.audio2_conv = nn.Conv2d(64, 128, kernel_size=1)
        ...
        self.audio3_conv_decoder = nn.Conv2d(128, 448, kernel_size=1)
        
        self.audio_encoder4 = nn.Sequential(
            Conv2d(128, 192, kernel_size=3, stride=2, padding=1),
        ...
        self.audio_adapter_4_conv = nn.Conv2d(192, 192, kernel_size=1)  # Channel adjustment
        
        self.audio_adapter_decoder4 = nn.AdaptiveAvgPool2d((24, 48))   # Match decoder5 spatial dims
        self.audio_adapter_decoder4_conv = nn.Conv2d(192, 192, kernel_size=1)  # Channel adjustment
        
        self.audio_adapter_decoder3 = nn.AdaptiveAvgPool2d((48, 96))   # Match decoder5 spatial dims
        self.audio_adapter_decoder3_conv = nn.Conv2d(192, 192, kernel_size=1)  # Channel adjustment
        ...
        self.bottom_unet_conv4 = construct_encoder_layers(3, 640, 256, 1) # 256 (debottom4) + 192 (bottom4) = 448
        
        self.bottom_unet_decoder3 = construct_decoder_layers(3, 256, 256, 2)
        self.bottom_unet_conv3 = construct_encoder_layers(3, 640, 256, 1) # 256 (debottom3) + 192 (bottom3) = 448
        ...
        self.face4_cross_attn = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=192, nhead=4, dropout=0.1, activation='gelu'),
            num_layers=4
        )
        self.face4_proj = nn.Linear(576, 288)
        
        self.bottleneck_cross_attn = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=256, nhead=4, dropout=0.1, activation='gelu'),
            num_layers=4
        )
        self.bottleneck_proj = nn.Linear(144, 72)
        
        self.face_norm = nn.LayerNorm(192)  # For face features
        self.audio_norm = nn.LayerNorm(192)  # For audio features
    ...
    def forward(self, audio_sequences, face_sequences, step=None, training=True):
        ...
        audio4_decoder4_adapted = self.audio_adapter_decoder4(audio_4)
        audio4_decoder4_adapted = self.audio_adapter_decoder4_conv(audio4_decoder4_adapted)
        audio4_decoder4_adapted = self.audio_pos_encoder_decoder4(audio4_decoder4_adapted)
        
        audio3_decoder3_adapted = self.audio_adapter_decoder3(audio_3)
        audio3_decoder3_adapted = self.audio_adapter_decoder3_conv(audio3_decoder3_adapted)
        audio3_decoder3_adapted = self.audio_pos_encoder_decoder3(audio3_decoder3_adapted)
        ...
        encoder1_to_encoder2_residual = self.encoder1_to_encoder2_pool(bottom_enc1)  # Downsample spatially
        encoder1_to_encoder2_residual = self.encoder1_to_encoder2_conv(encoder1_to_encoder2_residual)  # Adjust channels
        ...
        encoder2_to_encoder3_residual = self.encoder2_to_encoder3_pool(bottom_enc2)  # Downsample spatially
        encoder2_to_encoder3_residual = self.encoder2_to_encoder3_conv(encoder2_to_encoder3_residual)  # Adjust channels
        
        bottom_down2 = self.bottom_unet_down2(bottom_enc2)
        
        bottom_enc3 = self.bottom_unet_encoder3(bottom_down2 + encoder2_to_encoder3_residual)
        encoder3_to_encoder4_residual = self.encoder3_to_encoder4_pool(bottom_enc3)
        encoder3_to_encoder4_residual = self.encoder3_to_encoder4_conv(encoder3_to_encoder4_residual)  # Adjust channels
        
        bottom_down3 = self.bottom_unet_down3(bottom_enc3)
        
        bottom_enc4 = self.bottom_unet_encoder4(bottom_down3 + encoder3_to_encoder4_residual)
        encoder4_to_encoder5_residual = self.encoder4_to_encoder5_pool(bottom_enc4)
        encoder4_to_encoder5_residual = self.encoder4_to_encoder5_conv(encoder4_to_encoder5_residual)  # Adjust channels
        
        bottom_down4 = self.bottom_unet_down4(bottom_enc4)
        bottom_down4 = self.bottom_unet_pos_encoder4(bottom_down4)
        
        bottom_down4_to_bottleneck = self.bottom_down4_pool(bottom_down4)
        bottom_down4_to_bottleneck = self.bottom_down4_conv(bottom_down4_to_bottleneck)
        ...
        # For audio features  
        flat_audio5 = audio4_adapted.permute(0, 2, 3, 1)  # [B, H, W, C]
        flat_audio5 = self.audio_norm(flat_audio5)
        flat_audio5 = flat_audio5.permute(0, 3, 1, 2).flatten(2).permute(0, 2, 1)  # [B, seq_len, features]

        # Concatenate along sequence dimension
        face4_combined_sequence = torch.cat([flat_face4_down, flat_audio5], dim=1)  # [10, 576, 192]
        
        # Process through transformer
        face4_fused = self.face4_cross_attn(face4_combined_sequence)  # [10, 576, 192]
        ...
        face4_fused = face4_fused.view(batch_size, channels, height, width)  # [10, 192, 12, 24]
        
                        
        bottom_enc5 = self.bottom_unet_encoder5(bottom_down4 + face4_fused)
        bottom_down5 = self.bottom_unet_down5(bottom_enc5 + encoder4_to_encoder5_residual)
                
        bottom_bottleneck = self.bottom_unet_bottleneck(bottom_down5 + bottom_down4_to_bottleneck)
        ...
        bottom_cat4 = torch.cat([bottom_de4, bottom_enc4, audio4_decoder4_adapted], dim=1)
        bottom_cat4 = self.bottom_unet_conv4(bottom_cat4)
        
        bottom_de3 = self.bottom_unet_decoder3(bottom_cat4)
        bottom_cat3 = torch.cat([bottom_de3, bottom_enc3, audio3_decoder3_adapted], dim=1)
        print('The bottom_cat3', bottom_cat3.shape)
